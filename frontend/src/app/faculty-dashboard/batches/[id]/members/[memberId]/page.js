...
export default function FacultyMemberDetailPage({ params }) {
  const { user } = useAuth();
  ...
  // memberId format: "faculty-<uuid>" or "student-<uuid>"
  const [memberRole, memberId_raw] = memberId.split(/-(.+)/);

  useEffect(() => {
    if (user === null) router.push("/login");
    else if (user?.user_metadata?.role !== "faculty") router.push("/student-dashboard");
    else fetchMember();
  ...
  if (!member) {
    return (
      <div className="min-h-[calc(100vh-6rem)] w-full flex flex-col items-center justify-center gap-4">
        ...
        <Link href={`/faculty-dashboard/batches/${batchId}/members`} className="text-sm underline text-foreground/60">
          Back to Members
        </Link>
      </div>
    );
  ...
  const department = isFaculty ? member.department : member.department;
