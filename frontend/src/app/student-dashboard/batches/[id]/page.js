  ...
  return (
    <div className="min-h-screen w-full relative overflow-hidden bg-background">
      ...
      <div className="relative z-10 w-full max-w-6xl mx-auto p-6 md:p-10">
        {/* Breadcrumbs */}
        <div className="mb-8 flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.2em] text-foreground/30 animate-in fade-in slide-in-from-left-4 duration-500">
           <Link href="/student-dashboard" className="hover:text-foreground transition-colors">Dashboard</Link>
           <ChevronRight className="w-3 h-3" />
           <span className="text-foreground/60">{batch?.name || "Batch Details"}</span>
        </div>

        {/* Back Button */}
        <Link 
          href="/student-dashboard" 
          className="inline-flex items-center gap-2 mb-6 text-xs font-medium text-foreground/40 hover:text-foreground transition-all group"
        >
        ...
        <div className="mb-12 flex flex-col md:flex-row md:items-end justify-between gap-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
          <div className="space-y-2">
            <div className="flex items-center gap-2 mb-1">
               ...
               <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-foreground/40">Batch Portal</span>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-6 pt-2">
              <div className="flex items-center gap-2 bg-secondary/5 border border-border/30 px-3 py-1.5 rounded-xl backdrop-blur-sm">
                ...
                <span className="text-sm font-medium text-foreground/80">{totalPoints} <span className="text-foreground/40 font-light">Points Earned</span></span>
              </div>
              {pendingCount > 0 && (
                <div className="flex items-center gap-2 bg-secondary/5 border border-border/30 px-3 py-1.5 rounded-xl backdrop-blur-sm">
                  <div className="w-2 h-2 rounded-full bg-yellow-500 animate-pulse" />
                  <span className="text-sm font-medium text-foreground/80">{pendingCount} <span className="text-foreground/40 font-light">Pending Review</span></span>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="mt-20 p-10 border border-border/30 border-dashed rounded-[2.5rem] bg-secondary/5 backdrop-blur-sm flex flex-col items-center justify-center text-center group hover:border-border/60 transition-all duration-500">
           <div className="w-16 h-16 rounded-full bg-background border border-border/50 flex items-center justify-center mb-4 group-hover:scale-110 transition-transform">
              <Award className="w-8 h-8 text-foreground/20" />
           </div>
           <h3 className="text-lg font-display font-medium text-foreground/60 mb-2">Track your growth</h3>
           <p className="text-sm text-foreground/30 font-light max-w-md">Every submission brings you closer to your graduation requirement. Keep pushing!</p>
        </div>
      </div>
